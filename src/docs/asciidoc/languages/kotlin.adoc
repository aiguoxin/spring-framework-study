...
To retrieve a list of `com.agx.User` objects in Java, you would normally write the following:

[source,java,indent=0]
----
	Flux<com.agx.User> users  = client.get().retrieve().bodyToFlux(com.agx.User.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<com.agx.User>()
	// or (both are equivalent)
	val users : Flux<com.agx.User> = client.get().retrieve().bodyToFlux()
----
...
NOTE: Spring Boot is based on com.agx.JavaConfig and
...
class UserHandler(builder: WebClient.Builder) {
	...
	suspend fun listView(request: ServerRequest): ServerResponse =
			...
			client.get().uri("...").awaitExchange().awaitBody<com.agx.User>()))
...
* `equals()` and `hashCode()`
* `toString()` of the form `"com.agx.User(name=John, age=42)"`
